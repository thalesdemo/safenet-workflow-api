...
COPY jar/safenet-workflow-api-${APP_VERSION}.jar ./safenet-workflow-api.jar
COPY tools/keygen-1.0.jar ./tools/keygen.jar
COPY tools/begin.sh .
RUN chmod 700 /app/begin.sh && \
    chmod 700 /app/tools
RUN chmod 600 /app/tools/keygen.jar && \
    chmod 600 /app/safenet-workflow-api.jar

# Create the custom log directory and change ownership and permissions
RUN mkdir -p /var/log/workflow-api && \
    chown appuser:appuser /var/log/workflow-api && \
    chmod 755 /var/log/workflow-api
...
CMD ["java", "-jar", "/app/safenet-workflow-api.jar"]